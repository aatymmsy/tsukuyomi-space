...
const LLM_PRESETS = {
  ...
  deepseek: { label: 'DeepSeek', apiUrl: 'https://api.deepseek.com/chat/completions', model: 'deepseek-flash' },
...
function detectLLMProvider(apiUrl = '', modelName = '') {
  ...
  // SiliconFlow also serves models named "deepseek-ai/...", so its hostname has
  // to be checked before the generic deepseek substring rule below.
...
function providerStaticModelOptions(provider) {
  ...
  const options = presets
    .filter((preset) => detectLLMProvider(preset.apiUrl, preset.model) === provider && preset.model)
    .map((preset) => ({
      id: preset.model,
      nativeId: preset.model,
      // Include the concrete model id: a provider label alone made the model
      // impossible to find in the picker.
  ...
  // Also list the provider's commonly used ids so they can be picked before the
  // OpenRouter catalog has been synced.
  const seen = new Set(options.map((item) => item.nativeId));
  for (const model of MODEL_RECOMMEND_PATTERNS[provider] || []) {
    const id = String(model || '').trim();
    if (!id || seen.has(id)) continue;
    seen.add(id);
    options.push({
      id,
      nativeId: nativeModelId(id, provider),
      label: id,
      detail: '\u5e38\u7528\u6a21\u578b',
      contextLength: 0,
      created: 0,
...
function loadDiaryArchive() {
  ...
  // A backup can carry several personas; surface them all so one can be chosen.
  diary.personaList = listPersonaPrompts(archive).map((item) => ({
    id: item.id,
    label: item.label,
    name: item.name,
    tags: item.tags.join('、'),
  ...
  diary.activePersonaId = activePersonaId(archive);
...
function selectDiaryPersona(id) {
  try {
    selectPersonaPrompt(id);
    loadDiaryArchive();
    showToast(`已切换人设：${diary.personaName || id}（${id}）`);
  } catch (error) {
    showToast(`切换人设失败：${error.message}`);
  }
}
    ...
    <details class="room-advanced-settings" :open="globalAdvancedOpen" @toggle="globalAdvancedOpen = $event.currentTarget.open">
      ...
      <article id="room-diary-settings" class="room-settings-card">
        ...
        <div class="diary-archive-summary">
          ...
          <span class="field-hint">人设：{{ diary.personaList.length }} 个</span>
        ...
        <div v-if="diary.personaList.length" id="diary-persona-picker" class="form-grid">
          <label>当前使用的人设
            <select :value="diary.activePersonaId" @change="selectDiaryPersona($event.target.value)">
              <option v-for="item in diary.personaList" :key="item.id" :value="item.id">
                {{ item.label }}{{ item.tags ? ` · ${item.tags}` : '' }}
              </option>
            </select>
          </label>
          <p class="field-hint">
            存档里有 {{ diary.personaList.length }} 个人设，切换后房间里的称呼、说话方式和日记口吻都会跟着变。当前：{{ diary.personaName || '未命名' }}。
          </p>
        </div>
