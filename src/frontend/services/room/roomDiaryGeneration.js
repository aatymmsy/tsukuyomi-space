...
/**
 * Generates the diary through the site's restricted backend bridge.
 *
 * Used when the provider blocks direct browser requests (CORS). The backend
 * only forwards to allow-listed provider hosts, so this stays narrow.
 */
async function generateDiaryViaProxy({ settings, systemPrompt, userPrompt }) {
  ...
  if (!response.ok || !result.success) {
    throw new Error(`日记生成失败：${result.message || `HTTP ${response.status}`}`);
  }
  const reply = String(result.data?.reply || result.data?.text || '').trim();
  if (!reply) throw new Error('日记生成失败：模型没有返回内容。');
  return reply;
}
...
export async function generateDiaryEntry(turns, {
  ...
  if (settings.useProxy && !settings.useLocalOllama) {
    // Some providers refuse browser connections (CORS), so the diary request is
    // relayed through the site's restricted backend bridge like normal chat.
    reply = await generateDiaryViaProxy({ settings, systemPrompt, userPrompt });
  } else {
