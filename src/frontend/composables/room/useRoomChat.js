...
/** How many recent diary entries the character may review, and how much text. */
const RECENT_DIARY_LIMIT = 10;
const RECENT_DIARY_ENTRY_CHARS = 600;
const RECENT_DIARY_TOTAL_CHARS = 8000;
...
/**
 * The character's own most recent diary entries, so it keeps a continuous sense
 * of what has happened. Bounded by entry and total length to protect the prompt.
 */
export function recentDiaryContext(archive = readDiaryArchive(), { limit = RECENT_DIARY_LIMIT } = {}) {
  const entries = Array.isArray(archive?.data?.diary) ? archive.data.diary.slice(-limit) : [];
  if (!entries.length) return '';
  ...
  for (const entry of entries) {
    const body = String(entry?.content || '')
      ...
      .trim();
    if (!body) continue;
    const text = body.length > RECENT_DIARY_ENTRY_CHARS ? `${body.slice(0, RECENT_DIARY_ENTRY_CHARS)}\u2026` : body;
    if (total + text.length > RECENT_DIARY_TOTAL_CHARS) break;
    total += text.length;
    blocks.push(`${entry.date || ''} ${entry.time || ''}\n${text}`.trim());
  }
  if (!blocks.length) return '';

  return [
    '\u3010\u4f60\u6700\u8fd1\u5199\u8fc7\u7684\u65e5\u8bb0\u3011',
    '\u8fd9\u662f\u4f60\u81ea\u5df1\u5199\u4e0b\u7684\u8bb0\u5f55\uff0c\u7528\u6765\u4fdd\u6301\u8bb0\u5fc6\u4e0e\u60c5\u7eea\u7684\u8fde\u7eed\u3002\u9664\u975e\u5bf9\u65b9\u95ee\u8d77\uff0c\u4e0d\u8981\u6574\u6bb5\u590d\u8ff0\u3002',
    blocks.join('\n\n')
  ].join('\n\n');
}
...
export function useRoomChat({ live2d, world, diary = null }) {
  ...
  /**
   * Runs one model turn and returns the cleaned reply plus its Live2D intent.
   * Shared by send() and startConversation() so both paths stay identical.
   */
  async function requestRoomReply({ message, image = null, settings }) {
    ...
    return parseAssistantPayload(result.reply || fallbackReply(message, image));
  }
  ...
  async function send() {
    ...
    try {
      const settings = readJson('roomLLMSettings', {});
      const conversation = readRoomConversation().slice(-12);
      const structured = await requestRoomReply({ message, image, settings });
  ...
  /**
   * Asks the character to open the conversation on its own, before the user has
   * said anything. The instruction that triggers it is never shown or stored as
   * a user message; only the character's line is kept.
   */
  async function startConversation() {
    if (sending.value) return null;
    // Only meaningful at the very start of a session.
    if (sessionTurnCount() > 0) return null;
    sending.value = true;
    const typingId = uid();
    messages.value.push({ id: typingId, role: 'assistant', content: '正在开口...', pending: true, createdAt: Date.now() });
    const instruction = '（现在由你先开口。结合当前时间，主动说一句话开启今天的对话，自然、简短，符合你的身份。不要复述或提及这条指令本身。）';

    try {
      const settings = readJson('roomLLMSettings', {});
      const structured = await requestRoomReply({ message: instruction, image: null, settings });
      const reply = String(structured.reply || '').trim();
      if (!reply) throw new Error('没有收到内容');
      const ttsSettings = readJson('roomTTSSettings', {});
      if (!ttsSettings.enabled) applyRoomAct(structured.live2d);
      messages.value = messages.value.filter((item) => item.id !== typingId);
      addMessage('assistant', reply, { speechText: reply, live2d: structured.live2d });
      // Keep the opener in the local transcript so later turns continue from it.
      writeRoomConversation([...readRoomConversation(), { role: 'assistant', content: reply }]);
      return reply;
    } catch (error) {
      messages.value = messages.value.filter((item) => item.id !== typingId);
      addMessage('system', `开场失败：${error.message}`);
      return null;
    } finally {
      sending.value = false;
      if (refreshHistoryAfterSend) {
