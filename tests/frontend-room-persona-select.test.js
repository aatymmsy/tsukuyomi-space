...
describe('a real exported archive (opt-in)', () => {
    // Set ROOM_ARCHIVE_PATH to a real export to run this. The path is never
    // committed, so no personal file names or directories end up in the repo.
    const file = process.env.ROOM_ARCHIVE_PATH || '';

    it('reads every persona type in the real backup', { skip: !file || !fs.existsSync(file) }, () => {
        const { archive } = loadArchive();
        archive.importDiaryArchive(fs.readFileSync(file, 'utf8'));

        const list = archive.listPersonaPrompts();
        assert.ok(list.length >= 10, 'expected at least 10 personas, got ' + list.length);

        const ids = Array.from(list, (item) => item.id);
        for (const expected of ['sister-null', 'sister-dilei', 'sister-tutor', 'sister-kemonomimi', 'sister-kemonomimi-cat']) {
            assert.ok(ids.includes(expected), 'missing persona ' + expected);
        }
        // Labels must be distinguishable despite the shared name.
        assert.equal(new Set(Array.from(list, (item) => item.label)).size, list.length);

        // Switching must actually change what the room would use.
        archive.selectPersonaPrompt('sister-kemonomimi-cat');
        assert.match(archive.activePersonaPrompt().data.tags.join(','), /伶鼬/);
        archive.selectPersonaPrompt('sister-tutor');
        assert.match(archive.activePersonaPrompt().data.tags.join(','), /苏格拉底/);
    });
});
